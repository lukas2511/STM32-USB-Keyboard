...
int main(int ac, char** av) {
	...
	if (sl != NULL) {
		printf("ST-Linky Communication Daemon Thing :: Created by Lukas Schauer\n");
		printf("Based on ST-Linky proof-of-concept terminal :: Created by Necromant for lulz\n");
		stlink_version(sl);
...
//		printf("cpuid:part = %#x, rev = %#x\n", cpuid.part, cpuid.revision);
		...
		printf("OKAY HERE COMES THE GOOD PART!\n\n\n");

		struct stat statthing;
		int fifo;

		if (stat("/tmp/keys.fifo", &statthing) != 0) mkfifo("/tmp/keys.fifo", 0666);
		fifo = open("/tmp/keys.fifo", O_RDONLY);

		while(1) {
			sig_process();
			tmp = read(fifo, txbuf, 11);
			if(tmp>0) {
